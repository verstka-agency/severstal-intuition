...
const base64setting = (setting: Record<string, string>) => {
    ...
    function w(e: Record<string, string>) {
    ...
    var t = function (e) {
        return w(e).reduce((function (t: any, n) {
            return t[n] = e[n], t
        }), {})
    }(setting)
...
const LoginVKID: FC<ILoginVKID> = (props) => {
    const {
        id,
        redirect,
        state,
    ...
    useEffect(() => {
        ...
        const handler = (event: any) => {
            ...
            switch (handler) {
                case "VKSDKOneTapResizeFrame":
                    handlerResizeFrame(params)
                    break
                case "VKSDKOneTapAuthDataLoaded":
                    handlerDataLoader(params)
                    break
                case "VKSDKOneTapAuthLoginSuccess":
                    handlerLoginSuccess(params)
                    break
                case "VKSDKButtonOneTapAuthShowLoginOptions":
                    handlerAlternativeLogin()
                    break
                case "VKSDKOneTapAuthFullAuthNeeded":
                case "VKSDKOneTapAuthPhoneValidationNeeded":
                case "VKSDKButtonOneTapAuthShowLogin":
                    handlerWindowAuth()
                    break
            }
        }
